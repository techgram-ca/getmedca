...
export function PlatformSettingsForm({ searchRadiusKm, flatDeliveryFee, slaMinutes }: { searchRadiusKm: number; flatDeliveryFee: number; slaMinutes: number }) {
  const router = useRouter();
  const [v, setV] = useState({ searchRadiusKm, flatDeliveryFee, slaMinutes });
  ...
  return (
    <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); start(async () => { const r = await savePlatformSettings(v); if (r.ok) { toast.success("Settings saved"); router.refresh(); } else toast.error(r.error); }); }}>
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Field label="Flat delivery fee (CAD)" htmlFor="fee" hint="Charged to pharmacies per delivered order."><Input id="fee" type="number" min={0} step="0.01" value={v.flatDeliveryFee} onChange={(e) => setV({ ...v, flatDeliveryFee: Number(e.target.value) })} /></Field>
