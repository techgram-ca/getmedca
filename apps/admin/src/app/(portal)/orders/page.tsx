...
export default async function OrdersPage({ searchParams }: { searchParams: Promise<{ status?: string; pharmacy?: string; q?: string; from?: string; to?: string }> }) {
        ...
        <div className="surface overflow-hidden">
          <Table>
            ...
            <TBody>
              {rows.map((o) => (
                <TR key={o.id}>
                  <TD className="font-mono font-medium">{shortId(o.id)}<div className="text-[10px] uppercase text-ink-400">{o.order_type}</div></TD>
