...
export function RetryDelivery({ orderId, attempt }: { orderId: string; attempt: number }) {
    ...
    return (
      <div>
        ...
        <p className="mt-2 text-xs text-ink-500">
          This is delivery attempt {attempt}. The failed attempt has already been charged; the next one is charged separately.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <Alert tone="info">
        The order goes back to <strong>ready for delivery</strong> and GetMed assigns a driver. Attempt {attempt + 1} is billed
        on its own — the attempt that failed stays on your invoice.
      </Alert>
      <Field label="What changed?" htmlFor="retry-note" optional hint="Shown to GetMed support on the escalation.">
        <Textarea
          id="retry-note"
          rows={3}
          maxLength={500}
