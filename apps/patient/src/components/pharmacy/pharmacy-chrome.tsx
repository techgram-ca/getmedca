 ...
 */
export function PharmacyHeader({ pharmacy, links = [] }: { pharmacy: ChromePharmacy; links?: { href: string; label: string }[] }) {
    ...
    <header className="sticky top-0 z-50 border-b border-ink-200 bg-white/92 backdrop-blur-[16px]">
      <div className="mx-auto flex h-16 max-w-[1200px] items-center justify-between gap-4 px-6">
        ...
        <nav className="hidden items-center gap-6 md:flex" aria-label="Pharmacy">
          ...
          <Button asChild size="sm"><Link href={`/order/new?pharmacyId=${pharmacy.id}`}>Order prescription</Link></Button>
        </nav>
      ...
      {open ? (
        <div className="flex flex-col gap-4 border-t border-ink-200 bg-white px-6 py-4 md:hidden animate-fade-in">
          ...
          <Button asChild size="sm" className="w-fit"><Link href={`/order/new?pharmacyId=${pharmacy.id}`}>Order prescription</Link></Button>
        </div>
      ) : null}
    </header>
  );
...
export function StickyOrderBar({ pharmacy, className }: { pharmacy: ChromePharmacy; className?: string }) {
  return (
    <div className={cn("fixed inset-x-0 bottom-0 z-40 border-t border-ink-200 bg-white/95 px-4 py-3 backdrop-blur md:hidden", className)}>
      <div className="flex gap-2">
        <Button asChild className="flex-1"><Link href={`/order/new?pharmacyId=${pharmacy.id}`}>Order prescription</Link></Button>
